  ...
  fetch(configUrl)
    ...
    .then((config) => {
      ...
      function captureMap() {
        ...
        const mapWidthInInches = 8; // Width of the map on paper in inches
        const mapHeightInInches = 7; // Height of the map on paper in inches
        const mapWidthInPixels = mapWidthInInches * printDPI;
        const mapHeightInPixels = mapHeightInInches * printDPI;

        // Desired map scale: 1 inch = 100 feet
        const scale = (100 * 12) / printDPI; // 100 feet / (300 pixels/inch) -> scale in map units per pixel
